...
def add_manual_fire(frp_map, lat_map, lon_map, flat, flon, flvl):
    logger.info(f"Adding {len(flvl)} manual fire points")
    for i in np.arange(len(flvl)):
        dis = (lat_map - flat[i]) ** 2 + (lon_map - flon[i]) ** 2

        if frp_map[dis == np.min(dis)] == 0:
            frp_map[dis == np.min(dis)] = flvl[i]
            logger.debug(f"Added manual fire at lat={flat[i]}, lon={flon[i]} with level={flvl[i]}")

    logger.info("Manual fire addition completed")
    return frp_map
...
def main_driver(initial_hour, forecast_hour, f_input, f_output, lat_lim, lon_lim):
    ...
    logger.debug(f"FRP threshold: {frp_thres}, fire size: {fsize}")
    ...
    if forecast_hour == 0:  # initial fire
        ...
        logger.info(f"Initial fire map loaded: time={time}, LAT shape={LAT.shape}, "
                   f"FRP non-zero pixels: {(FRP != 0).sum()}")
        ...
        if manual_fire == 1:
            ...
            logger.info(f"Manual fire coordinates: lat={man_lat}, lon={man_lon}, levels={man_lvl}")
            FRP = add_manual_fire(FRP, LAT, LON, man_lat, man_lon, man_lvl)
            logger.warning("Manual fire added!")
    ...
    logger.debug("FRP data copied to input array")
